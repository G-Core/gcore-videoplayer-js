...
import { TimePosition, reportError } from '@gcorevideo/player';
...
export class VastAds extends UICorePlugin {
  ...
  constructor(core: Core) {
    ...
    if (cfg.middleroll?.data?.length > 0) {
      cfg.middleroll.data = cfg.middleroll.data
        .filter(({ startTimePercent }: AdRollItem) => startTimePercent >= 10 && startTimePercent <= 90)
        .sort((a: AdRollItem, b: AdRollItem) => a.startTimePercent - b.startTimePercent);
    }
  ...
  private onPlaybackTimeUpdate(e: TimePosition) {
    ...
    if (this._options.VastAds.repeatableroll && this.playback.getPlaybackType() === 'live') {
      try {
        if (!(this.playback.el as HTMLMediaElement).played || !(this.playback.el as HTMLMediaElement).played.length) {
          return;
        }
        let startTime = (this.playback.el as HTMLMediaElement).played.start(0) || this.startTimeRepeatableRoll;

        if (!this.startTimeRepeatableRoll) {
          if (!startTime) {
            this.startTimeRepeatableRoll = startTime = (this.playback.el as HTMLMediaElement).currentTime;
          }
        }

        const currentTime = (this.playback.el as HTMLMediaElement).currentTime - startTime + this.startTimeRepeatableRollGap;
  ...
  private initializeRollManager({ type, count }: { type: AdRollType; count?: number }) {
    this.destroyRoll();
  ...
  private findCloserAdvertisement(roll: {data: AdRollItem[]} | undefined, key: 'startTime' | 'startTimePercent', counter: number, time: number) {
    if (!roll) {
      return -1;
    }
    ...
    const getNumber = (arr: AdRollItem[], searchNum: number) =>
      arr.find(it => Math.abs(it[key] - searchNum) === Math.min(...arr.map(it => Math.abs(it[key] - searchNum))));
  ...
  private _validateData(roll: { data: AdRollItem[]; oneByOne?: boolean }): boolean {
    try {
      if (roll.data.length) {
  ...
  private _pauserollListeners() {
    ...
    this.listenTo(this.playback, Events.PLAYBACK_PLAY, () => {
      ...
      if (this.playback?.getPlaybackType() === 'live' && wsPlugin?.state === 'live') {
  ...
  private containerChanged() {
    ...
    this._volume = this.core &&
      this.core.mediaControl &&
      this.core.mediaControl.volume !== null &&
      this.core.mediaControl.volume !== undefined &&
      !isNaN(this.core.mediaControl.volume) ? this.core.mediaControl.volume : 80;
    ...
    try {
      ...
      if (!this._clonePlaybackEvents) {
        for (const id in this._container._listeningTo) {
          this._clonePlaybackEvents = Object.assign({}, (this._container._listeningTo as any)[id]._events);
  ...
  _startListening() {
    try {
      assert(this._container, 'container is not defined');
      for (const id in this._container._listeningTo) {
        (this._container._listeningTo as any)[id]._events = Object.assign({}, this._clonePlaybackEvents);
      }
