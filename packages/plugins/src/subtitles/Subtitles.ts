...
export class Subtitles extends UICorePlugin {
  ...
  show() {
    this.isShowing = true;
    this.renderIcon();
    if (this.currentContainer && isFullscreen(this.currentContainer.el) && this.currentLevel && this.currentLevel.mode && Browser.isiOS) {
      this.$string.hide();
  ...
  render() {
    ...
    if (
      this.core.mediaControl.$subtitlesSelector?.find('span.subtitle-text').length > 0
    ) {
      this.renderIcon();
    }
  ...
  levelElement(id?: string): ZeptoResult {
    return (this.$('.subtitles ul a' + (id ? '[data-subtitles-select="' + id + '"]' : '')) as ZeptoResult).parent();
  }
          ...
          if (cues && cues.length) {
            for (const cue of cues) {
              if (currentTime >= cue.startTime && currentTime <= cue.endTime) {
                subtitleText += (cue as VTTCue).getCueAsHTML().textContent + '\n';
              }
            }
          }
  ...
  private renderIcon() {
    ...
    this.core.mediaControl.$subtitlesSelector.find('span.subtitle-text').html(icon);
  }
}
